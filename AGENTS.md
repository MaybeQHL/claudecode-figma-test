...
└── app/
    ...
    └── worldcup/
        ...
        ├── _lib/           # 私有目录，不生成路由
        │   ├── data.ts     # Mock 数据与类型（模块化组件使用）
        │   └── motion.ts   # Framer Motion 变体
        └── _components/    # 私有目录，模块化拆分（尚未接入 page.tsx）
            ├── sections/   # Hero, MatchSection, PenaltyGame 等
            └── ui/         # GlassCard, NeonButton, CountUp 等
```

**修改活动页时**：优先改 `src/app/worldcup/page.tsx`（当前实际渲染）。若做模块化重构，改用 `_components/` 下的 section/ui 组件，并复用 `_lib/data.ts` 与 `_lib/motion.ts`。
...
4. **数据**：当前为 Mock 静态数据；模块化组件的数据在 `src/app/worldcup/_lib/data.ts`，新接口接入时优先扩展该文件的类型与导出
